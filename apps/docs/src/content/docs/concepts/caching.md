...
The scope of deployment distribution is inferred from connected domains and their DNS [configuration](/concepts/dns).
...
are fetched from a vault. After a period of inactivity, a deployment is evicted if it is not connected to a custom
domain. Similarly, associated environment variables are evicted if unused.

Execution environments - sandboxes, due to their more considerable use of system resources, are evicted after a shorter
period of inactivity. A request hitting a deployment without a sandbox will not trigger a whole new initialisation of a
