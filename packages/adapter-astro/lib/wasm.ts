// Taken from MIT licensed https://github.com/withastro/adapters/blob/main/packages/cloudflare/src/utils/wasm-module-loader.ts
  ...
  return {
    ...
    config(_, __) {
      ...
      return {
        ...
        build: {
          rollupOptions: { external: /^__WASM_ASSET__.+\.wasm\.mjs$/i },
        },
      };
    },
...
export default wasmModule;
    ...
    renderChunk(code, chunk, _) {
      ...
      const final = code.replaceAll(
        /__WASM_ASSET__([A-Za-z\d]+).wasm.mjs/g,
        (s, assetId) => {
